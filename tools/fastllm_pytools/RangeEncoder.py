...
def range_encode(data, prob = None):
	...
	outstream.write(struct.pack(">i",257))
	for i in range(257):
		outstream.write(struct.pack(">i",prob[i]))
	...
	for inpos in range(insize+1):
		...
		while enc.hasbyte():
			buf[0]=enc.getbyte()
			outstream.write(buf)
	stream_size = len(outstream.getvalue())
	print(f'The size of the input data is: {insize} bytes')
	print(f'The size of the byte stream is: {stream_size} bytes')
	# outstream.seek(0)
	return outstream
...
if __name__=="__main__":
	...
	if mode=="-d":
		...
		# 	thread = Process(target=range_encode, args=(data[i * (insize // thread_num) : (i + 1) * (insize // thread_num)],))
		# 	thread.start()  # 启动线程
		...
		# data = np.fromfile("input_file.bin", dtype=np.uint8)
		# data = np.random.randint(0, 256, 1000000)
		data = np.random.normal(128, 10, 1000000)
		data = np.round(data).astype(np.uint8)
		data = np.clip(data, 0, 256)
		# print(data)
		data.astype(np.uint8).tofile("input_file.bin")
		outstream = range_encode(data)
		outstream.seek(0)
		with open("output_file.bin", "wb") as file:
    	# 从 BytesIO 对象读取内容并写入文件
		...
		instream = open("output_file.bin", "rb")
		probsize = struct.unpack(">i",instream.read(4))[0]
		prob = [0] * probsize
		for i in range(probsize):
			prob[i] = struct.unpack(">i",instream.read(4))[0]
		
		print(prob)
		outsize = struct.unpack(">i",instream.read(4))[0]
		insize = os.path.getsize("output_file.bin")
		buf=bytearray(1)
		outstream = io.BytesIO()

		dec=RangeEncoder(False)
		inpos, outpos=4 + 4 * probsize + 4, 0
		while outpos < outsize:
			print(f'Processing {outpos}/{outsize}', end='\r')
		...
		outstream.seek(0)
		res = outstream.read()
		array = np.frombuffer(res, dtype=np.uint8)
		print(array)
		print(data)
	# outstream.close()
	# instream.close()
